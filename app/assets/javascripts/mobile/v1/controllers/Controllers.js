...
function ImportControllers(appModule){
    ...
    appModule.controller('ProductDetailController',['$scope', '$routeParams', '$compile', function($scope, $routeParams, $compile){
        ...
        $scope.AddNewSpecificationRow = function(){
            ...
            var html_template = '<div class="row" id="row-id"> <div class="input-field col s3"> <input id="tesat" name="specification_values[]" placeholder="红" type="text" ><label for="tesat" class="active"> 规格</label> </div> <div class="input-field col s3"> <input id="tesst" name="specification_prices[]" placeholder="3.5" type="text"><label for="tesst" class="active"> 价格</label> </div><div class="input-field col s3"> <input id="id-for-stock" name="specification_stocks[]" placeholder="100" type="text" ><label for="id-for-stock" class="active"> 库存</label> </div> <div class="col s3"> <a class="btn-floating btn-large waves-effect waves-light red hover-btn" ng-click="RemoveSpecification(\'row-id\')">删</a> </div> </div>';
